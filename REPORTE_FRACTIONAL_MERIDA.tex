...
\definecolor{rojo}{RGB}{220,50,50}          % Rojo temporada alta
\definecolor{amarillo}{RGB}{230,180,50}     % Amarillo temporada media
\definecolor{azulclaro}{RGB}{100,180,220}   % Azul claro temporada baja
\definecolor{rosa}{RGB}{255,180,200}        % Rosa para gastos
...
% Cajas y destacados (cargar DESPUÉS de hyperref)
% Suprimir salida de comandos problemáticos de tcolorbox
\makeatletter
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Rendimiento indirecto: 20-25\% en valor de uso vs. hoteles
    \end{itemize}
\end{enumerate}
...
\begin{tikzpicture}[scale=0.85, transform shape]
    % Leyenda
    \node[fill=rojo, minimum width=0.6cm, minimum height=0.4cm] at (1,5.5) {};
    \node[anchor=west] at (1.4,5.5) {\small ALTA};
    
    \node[fill=amarillo, minimum width=0.6cm, minimum height=0.4cm] at (3,5.5) {};
    \node[anchor=west] at (3.4,5.5) {\small MEDIA};
    
    \node[fill=azulclaro, minimum width=0.6cm, minimum height=0.4cm] at (5,5.5) {};
    \node[anchor=west] at (5.4,5.5) {\small BAJA};
    
    % Ejes y etiquetas
    \draw[->] (0,0) -- (14,0) node[right] {\small Semana};
    \draw[->] (0,0) -- (0,5);
    
    % Labels de tickets
    \node[anchor=east] at (-0.2,0.5) {\small Ticket A};
    \node[anchor=east] at (-0.2,1.7) {\small Ticket B};
    \node[anchor=east] at (-0.2,2.9) {\small Ticket C};
    \node[anchor=east] at (-0.2,4.1) {\small Ticket D};
    
    % Marcadores de semanas (cada 5)
    \foreach \x in {0,5,...,50} {
        \draw (\x/4,0) -- (\x/4,-0.1);
        \node[anchor=north, font=\tiny] at (\x/4,-0.15) {\x};
    }
    
    % Ticket A (7 ALTA, 3 MEDIA, 3 BAJA)
    \fill[rojo] (0.25,0.3) rectangle (0.75,0.7);     % S1 ALTA
    \fill[amarillo] (2,0.3) rectangle (3,0.7);       % S8-12 MEDIA  
    \fill[rojo] (3.5,0.3) rectangle (5,0.7);         % S14-20 ALTA
    \fill[rojo] (6,0.3) rectangle (7,0.7);           % S24-28 ALTA
    \fill[amarillo] (8.5,0.3) rectangle (9.5,0.7);   % S34-38 MEDIA
    \fill[azulclaro] (10,0.3) rectangle (11,0.7);    % S40-44 BAJA
    \fill[rojo] (12,0.3) rectangle (13,0.7);         % S48-52 ALTA
    
    % Ticket B (7 ALTA, 3 MEDIA, 3 BAJA)
    \fill[rojo] (0.5,1.5) rectangle (1,1.9);         % S2 ALTA
    \fill[amarillo] (1.5,1.5) rectangle (2.5,1.9);   % S6-10 MEDIA
    \fill[azulclaro] (3,1.5) rectangle (4,1.9);      % S12-16 BAJA  
    \fill[rojo] (5,1.5) rectangle (6.5,1.9);         % S20-26 ALTA
    \fill[rojo] (8,1.5) rectangle (9,1.9);           % S32-36 ALTA
    \fill[azulclaro] (10.5,1.5) rectangle (11.5,1.9); % S42-46 BAJA
    \fill[rojo] (12.5,1.5) rectangle (13,1.9);       % S50 ALTA
    
    % Ticket C (6 ALTA, 4 MEDIA, 3 BAJA)
    \fill[amarillo] (0.75,2.7) rectangle (1.5,3.1);  % S3-6 MEDIA
    \fill[rojo] (2.5,2.7) rectangle (3.5,3.1);       % S10-14 ALTA
    \fill[azulclaro] (4.5,2.7) rectangle (6,3.1);    % S18-24 BAJA
    \fill[rojo] (7,2.7) rectangle (8,3.1);           % S28-32 ALTA
    \fill[amarillo] (9,2.7) rectangle (10,3.1);      % S36-40 MEDIA
    \fill[rojo] (11,2.7) rectangle (12,3.1);         % S44-48 ALTA
    \fill[amarillo] (13,2.7) rectangle (13.25,3.1);  % S52 MEDIA
    
    % Ticket D (6 ALTA, 4 MEDIA, 3 BAJA)
    \fill[rojo] (1,3.9) rectangle (2,4.3);           % S4-8 ALTA
    \fill[amarillo] (3.25,3.9) rectangle (4.5,4.3);  % S13-18 MEDIA
    \fill[azulclaro] (5.5,3.9) rectangle (7,4.3);    % S22-28 BAJA
    \fill[rojo] (7.25,3.9) rectangle (8.5,4.3);      % S29-34 ALTA  
    \fill[amarillo] (9.5,3.9) rectangle (10.5,4.3);  % S38-42 MEDIA
    \fill[rojo] (11.5,3.9) rectangle (12.5,4.3);     % S46-50 ALTA
    
\end{tikzpicture}
\caption{Distribución Anual de Semanas - Sistema de Rotación Serpiente}
\label{fig:distribucion-serpiente}
\end{figure}
...
\toprule
\textbf{Escenario} & \textbf{Ocup. ALTA} & \textbf{Ocup. MEDIA} & \textbf{Ocup. BAJA} & \textbf{Ing. Brutos} & \textbf{Ing. Netos} & \textbf{Rendimiento} \\
\midrule
...
\begin{axis}[
    ...
    ylabel={Valor (MXN)},
    xlabel={Escenario},
    symbolic x coords={Conservador, Moderado, Optimista},
    xtick=data,
    ymin=0,
    ymax=750000,
...
% Ingresos Brutos
\addplot[fill=turquesa!70, draw=turquesa] coordinates {
    (Conservador, 447020)
    (Moderado, 529060)
    (Optimista, 602280)
};
...
% Gastos Operativos  
\addplot[fill=rosa, draw=rosa!80!black] coordinates {
    (Conservador, 125166)
    (Moderado, 148137)
    (Optimista, 168638)
};
...
\legend{Ingresos Brutos, Gastos Operativos, Ingreso Neto}

% Anotaciones de ROI
\node[anchor=south, font=\tiny] at (axis cs:Conservador,450000) {\textbf{Rend/Ticket: \$80,464}};
\node[anchor=south, font=\tiny] at (axis cs:Conservador,480000) {Tasa: 8.94\%};

\node[anchor=south, font=\tiny] at (axis cs:Moderado,540000) {\textbf{Rend/Ticket: \$95,231}};
\node[anchor=south, font=\tiny] at (axis cs:Moderado,570000) {Tasa: 10.58\%};

\node[anchor=south, font=\tiny] at (axis cs:Optimista,620000) {\textbf{Rend/Ticket: \$108,410}};
\node[anchor=south, font=\tiny] at (axis cs:Optimista,650000) {Tasa: 12.05\%};

\end{axis}
\end{tikzpicture}
\caption{Proyección de Rendimientos Anuales por Escenario}
...
\begin{enumerate}
    \item \textbf{Opción 1 - Fondo común}: La semana 53 se renta automáticamente y el ingreso se divide equitativamente entre los 4 tickets
...
\newpage
\section{Glosario de Términos Financieros e Inmobiliarios}

\subsection{Términos Financieros}

\begin{description}[leftmargin=4cm,style=nextline]
    \item[Rendimiento] Ganancia o beneficio que produce una inversión en un periodo determinado, expresado usualmente como porcentaje anual. En este proyecto, se refiere a los ingresos por renta distribuidos a los copropietarios.
    
    \item[TIR (Tasa Interna de Retorno)] Métrica financiera que indica la rentabilidad de un proyecto considerando el valor del dinero en el tiempo. Equivale a la tasa de descuento que hace que el VPN sea igual a cero.
    
    \item[Retorno de Inversión] Sinónimo de rendimiento. Relación entre el beneficio obtenido y el capital invertido, expresado como porcentaje.
    
    \item[VPN (Valor Presente Neto)] Valor presente de los flujos de efectivo futuros de una inversión, descontados a una tasa específica, menos la inversión inicial.
    
    \item[Flujo de Efectivo] Movimiento de entrada y salida de dinero en un periodo determinado. En este contexto, incluye ingresos por renta menos gastos operativos.
    
    \item[Tasa de Ocupación] Porcentaje del tiempo que la propiedad está rentada versus el tiempo total disponible. Métrica clave para proyectar ingresos.
    
    \item[Punto de Equilibrio] Nivel de ocupación o ingresos necesario para cubrir exactamente todos los gastos operativos, sin ganancias ni pérdidas.
    
    \item[Capitalización] Proceso de reinvertir ganancias o constituir un fondo. En el proyecto, se refiere al fondo de reserva constituido con 5\% de utilidades netas anuales.
\end{description}

\subsection{Términos Inmobiliarios y Legales}

\begin{description}[leftmargin=4cm,style=nextline]
    \item[Fideicomiso] Figura jurídica donde un fiduciario (banco) administra un patrimonio (la propiedad) en beneficio de fideicomisarios (los copropietarios), según instrucciones del fideicomitente.
    
    \item[Fideicomisario] Persona que tiene derechos sobre el patrimonio del fideicomiso. En este proyecto, cada uno de los 4 copropietarios que posee un ticket.
    
    \item[Fiduciario] Institución bancaria que administra formalmente el patrimonio del fideicomiso y ejecuta las instrucciones del reglamento interno.
    
    \item[Usufructo] Derecho de usar y disfrutar de un bien ajeno con la obligación de conservarlo. Los copropietarios tienen usufructo sobre la propiedad durante sus semanas asignadas.
    
    \item[Copropietario] Persona que posee una fracción de la propiedad junto con otros propietarios. En el modelo, cada ticket representa 25\% de participación patrimonial.
    
    \item[Copropiedad] Régimen de propiedad donde varias personas tienen derechos sobre un mismo bien inmueble en proporciones definidas.
    
    \item[Ticket/Paquete] Unidad de participación fraccional que otorga 25\% de derechos patrimoniales y 13 semanas de uso anual.
    
    \item[Reglamento Interno] Documento legal que establece las reglas de operación, derechos y obligaciones de los copropietarios en el fideicomiso.
    
    \item[Derecho de Preferencia] También llamado "derecho de tanto". Prerrogativa de los copropietarios actuales para adquirir un ticket antes de que se venda a terceros.
    
    \item[Predial] Impuesto sobre la propiedad inmobiliaria, pagado anualmente al municipio. En Mérida se calcula sobre el valor catastral del inmueble.
\end{description}

\subsection{Términos Operativos del Proyecto}

\begin{description}[leftmargin=4cm,style=nextline]
    \item[Temporada Alta] Periodo de 26 semanas (Dic-Mar, Jul-Ago) con mayor demanda turística, mejores condiciones climáticas y tarifas máximas de renta.
    
    \item[Temporada Media] Periodo de 14 semanas (Abr-May, Oct-Nov) con demanda moderada y tarifas intermedias.
    
    \item[Temporada Baja] Periodo de 12 semanas (finales May, Jun, Sep) con menor demanda, época de lluvias y tarifas reducidas.
    
    \item[Sistema Serpiente] Método de distribución de semanas que invierte el orden de asignación en cada ronda (A-B-C-D, luego D-C-B-A), garantizando equidad entre copropietarios.
    
    \item[Rotación Anual] Cambio del orden inicial del sistema serpiente cada año para que ningún ticket tenga ventaja permanente a largo plazo.
    
    \item[Pax] Abreviatura del latín \textit{pax} (paz), utilizada en hotelería para indicar número de personas. La propiedad tiene capacidad de 4 pax.
    
    \item[Property Manager] Empresa o persona responsable de la gestión operativa de la propiedad: renta, mantenimiento, limpieza, atención a huéspedes.
    
    \item[OTA (Online Travel Agency)] Plataformas digitales de reservación turística como Airbnb, Booking.com, VRBO, Expedia, etc.
    
    \item[Tarifa Nocturna] Precio de renta por cada noche de hospedaje. Varía según temporada y demanda.
    
    \item[Check-in/Check-out] Horarios de entrada y salida de huéspedes. Estándar en Mérida: entrada 3pm, salida 11am.
    
    \item[Fondo de Reserva] Capital acumulado (equivalente a 6 meses de gastos operativos) para cubrir emergencias, reparaciones mayores o periodos de baja ocupación.
    
    \item[Gastos Operativos] Costos recurrentes de mantenimiento, servicios, comisiones, impuestos y administración de la propiedad. Proyectados en 28\% de ingresos brutos.
    
    \item[Distribución Proporcional] Método de reparto de utilidades donde cada copropietario recibe rendimientos solo por las semanas que efectivamente se rentaron de su ticket.
    
    \item[Semana 53] Semana adicional que ocurre en años bisiestos o ajustes de calendario. Se renta automáticamente y el ingreso se divide entre los 4 tickets.
\end{description}

\subsection{Acrónimos y Abreviaturas}

\begin{description}[leftmargin=4cm,style=nextline]
    \item[MXN] Peso mexicano (ISO 4217: MXN). Moneda de curso legal en México.
    
    \item[USD] Dólar estadounidense (ISO 4217: USD).
    
    \item[m²] Metro cuadrado, unidad de superficie de construcción.
    
    \item[SAT] Servicio de Administración Tributaria, autoridad fiscal mexicana.
    
    \item[PROFECO] Procuraduría Federal del Consumidor, organismo de protección al consumidor en México.
    
    \item[RFC] Registro Federal de Contribuyentes, clave fiscal única de personas físicas y morales en México.
    
    \item[NOM-029] Norma Oficial Mexicana que regula servicios de tiempo compartido.
    
    \item[CDMX] Ciudad de México (antes Distrito Federal).
\end{description}
